document.addEventListener("DOMContentLoaded", function() {
    ...
    function deleteFavorite(cityName) {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                const favoriteCard = document.querySelector(`.favorite-card[data-city="${cityName}"]`);
                if (favoriteCard) {
                    favoriteCard.remove();
                }
            } else {
    ...
    function fetchWeatherForDashboard(cityName, lat, lon) {
        fetch(`/get_weather?city_name=${cityName}&lat=${lat}&lon=${lon}`)
            ...
            .then(data => {
                if (!data.error) {
                    document.getElementById(`temp-${data.name}`).textContent = `Temperature: ${data.temp} °C`;
                    document.getElementById(`desc-${data.name}`).textContent = `Weather: ${data.description}`;
                    document.getElementById(`icon-${data.name}`).src = `http://openweathermap.org/img/wn/${data.icon}.png`;
                }
            })
            .catch(error => console.error("Error fetching weather data:", error));
    }
